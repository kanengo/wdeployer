...
var (
	...
	//	Tool:  "weaver standalone",
	//	Kill:  "weaver standalone (dashboard|deploy|logs|profile)",
	...
	Commands = map[string]*tool.Command{
		...
		//"status":    status.StatusCommand("weaver standalone", defaultRegistry),
		//"metrics":   status.MetricsCommand("weaver standalone", defaultRegistry),
		//"profile":   status.ProfileCommand("weaver standalone", defaultRegistry),
		//"purge":     tool.PurgeCmd(purgeSpec),
		//"version": itool.VersionCmd("weaver standalone"),
	}
)
