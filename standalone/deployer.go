...
func newDeployer(ctx context.Context, deploymentId string, config *StandaloneConfig, tmpDir string) (*deployer, error) {
	// Create the log saver.
	//logsDB, err := logging.NewFileStore(logDir)
	//if err != nil {
	//	return nil, fmt.Errorf("cannot create log storage: %w", err)
	//}
	printer := logging.NewPrettyPrinter()
	logger := slog.New(&wlogging.LogHandler{
		Opts: wlogging.Options{
			App:       config.App.Name,
			Component: "deployer",
			Weavelet:  uuid.NewString(),
			Attrs:     []string{"serviceweaver/system", ""},
		},
		Write: func(e *protos.LogEntry) { log(printer, e) },
	})
...
func (d *deployer) computeGroups() error {
	...
	components, _, err := bin.ReadComponentGraph(d.config.App.Binary)
	if err != nil {
		return fmt.Errorf("cannot read the call graph from the application binary: %w", err)
	}
	g, err := ensureGroup(components[0])
	if err != nil {
		return err
	}

	for i := 1; i < len(components); i++ {
		groups[components[i]] = g
	}
...
func (h *handler) subscribeTo(req *protos.ActivateComponentRequest) error {
	...
	if !req.Routed && h.g.name == target.name {
		// Route locally.
		routingInfo := &protos.RoutingInfo{Component: req.Component, Local: true}
		return h.envelope.UpdateRoutingInfo(routingInfo)
	}
...
func (d *deployer) readMetrics() []*metrics.MetricSnapshot {
	...
	for _, group := range d.groups {
		for _, enl := range group.envelopes {
			m, err := enl.GetMetrics()
			if err != nil {
				continue
			}
			ms = append(ms, m...)
...
func log(printer *logging.PrettyPrinter, e *protos.LogEntry) {
	if !logging.IsSystemGenerated(e) {
		fmt.Println("log entry", e.TimeMicros)
